...
- Admin có thể cấu hình PayOS settings từ khu vực admin.
...
- Mã hóa PayOS `ApiKey` và `ChecksumKey` trước khi lưu.
- Không bao giờ trả raw PayOS secrets về frontend.
...
3. PayOS configuration model, encryption boundary và admin settings API.
...
Approved decision: Cấu hình PayOS provider được lưu trong database, không dùng `appsettings` làm nguồn chính.

Approved decision: `ApiKey` và `ChecksumKey` phải được mã hóa trước khi lưu và UI chỉ được thấy dạng masked.
